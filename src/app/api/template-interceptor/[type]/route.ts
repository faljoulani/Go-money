...
export async function GET(request: Request, { params }: { params: Promise<{ type: string }> }) {
  ...
  // set used by count
  const templateStatisticsResponse = await RestClient.getTemplatesStatistics({
    templateNames: allNewEditorTemplateNames,
    additionalHeaders,
    additionalQueryParams,
  });
  templateStatisticsResponse.forEach((templateStat) => {
    const template = reactCategory?.Templates.find(
      (x) => x.Name === `${RENDERER_NAME}.${templateStat.Name}`,
    );
    if (template) {
  ...
  if (currentlyUsed && currentlyUsed.Templates.length === 1) {
    ...
    const foundSameReactTemplate = reactCategory.Templates.find(
