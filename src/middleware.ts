...
async function middlewareFrontend(request: NextRequest) {
  ...
  if (
    bypassHost ||
    cmsPaths.some((path) =>
      request.nextUrl.pathname.toUpperCase().startsWith(path.toUpperCase()),
    ) ||
...
function shouldBypassHost(request: NextRequest) {
  ...
  if (remoteValidationKey) {
    if (
      request.headers.has(headerBypassHostKey) &&
      request.headers.has(headerBypassHostValidationKey)
    ) {
...
function generateProxyRequest(request: NextRequest, bypassHost: string) {
  ...
  let resolvedHost =
    process.env.SF_PROXY_ORIGINAL_HOST ||
