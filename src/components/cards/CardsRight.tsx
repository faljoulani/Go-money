...
export async function CardsRight(props: WidgetContext<CardSectionEntity>) {
  ...
  if (parentSelection?.Content?.length) {
    try {
      const res = await RestClientForContext.getItems(parentSelection, {
        ...
        fields: [
          'Id', 'Title', 'Description', 'CtaText', 'CtaUrl','Eyebrow',
  ...
  const parentImageField = firstOrSelf(cardList?.Image);
  const parentImage = getImageUrl(parentImageField);
  const parentImageAlt = parentImageField?.AlternativeText || parentImageField?.Title || cardList?.Title;

  return (
    <section {...attrs} className="CardsRight-section">
      <div className="CardsRight-container">
        {parentImage && (
          <div className="CardsRight-bgImage">
            <img src={parentImage} alt={parentImageAlt} />
          </div>
        )}
        ...
        <div className="CardsRight-content">
          ...
          <div className="CardsRight-left">
            ...
            {cardList?.CtaText && cardList?.CtaUrl && (
              <a href={parseLink(cardList.CtaUrl)} className="CardsRight-button">
                {cardList.CtaText}
              </a>
            )}
          </div>
          ...
          <div className="CardsRight-right">
            {cards.map((item, i) => {
              ...
              return (
                <div key={item.Id || i} className="CardsRight-card">
                  {image && <img src={image} alt={alt} className="CardsRight-cardIcon" />}
                  <span className="CardsRight-cardTitle">{item.Title}</span>
                </div>
              );
            })}
          </div>
