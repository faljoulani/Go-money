...
export default function FooterLinks({ groups, className = '' }: Props) {
  ...
  return (
    <div className={`grid md:grid-cols-3 xs:grid-cols-2 gap-8 ${className} rtl:grid-col-reverse`}>
      {groups.map((group) => (
            ...
            {group.links.map((link) => {
              ...
              return (
                <li key={link.id}>
                  <Link
                    href={link.href}
                    className={` font-normal text-base leading-[100%] no-underline transition-colors align-middle ${'text-[#E0E0E0]'}`}
                  >
                    {link.title}
                  </Link>
                </li>
